...
const ENV_MAP = {
  ...
  'opencode.enableExa': 'OPENCODE_ENABLE_EXA',
...
function buildEnvShellStatements(getConfig, platform = process.platform) {
  const config = getConfig();
  const statements = [];
  for (const [key, envVar] of Object.entries(ENV_MAP)) {
    const value = config.get(key);
    if (value !== undefined && value !== null && value !== '') {
      ...
      if (platform === 'win32') {
        statements.push(`$env:${envVar}="${formatted}"`);
      } else {
        statements.push(`export ${envVar}="${formatted}"`);
      }
    }
  }
  return statements;
...
function joinTerminalCommand(statements, command, platform = process.platform) {
  if (statements.length === 0) {
    return command;
  }
  const separator = platform === 'win32' ? '; ' : ' && ';
  return statements.join(separator) + separator + command;
}

function buildTerminalCommand(getConfig, command, platform = process.platform) {
  return joinTerminalCommand(buildEnvShellStatements(getConfig, platform), command, platform);
}

function buildEnvExports(getConfig) {
  return buildEnvShellStatements(getConfig, process.platform);
}
