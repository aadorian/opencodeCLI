...
class AgentPanelProvider {
  ...
  openInEditor() {
    ...
    panel.onDidDispose(() => { this._editorPanel = null; });
    this._attach(panel);
  }

  _attach(webviewHost) {
    webviewHost.webview.options = { enableScripts: true };
    webviewHost.webview.html = this._getHtml();

    webviewHost.webview.onDidReceiveMessage(async msg => {
      switch (msg.type) {
        case 'send':
        case 'chip':
        ...
        case 'ready':
          ...
          break;
        case 'addContext':
          await this._handleAddContext(webviewHost.webview);
          break;
        case 'pickAgent':
          await this._handlePickAgent(webviewHost.webview);
          break;
        case 'pickModel':
          await this._handlePickModel(webviewHost.webview);
          break;
      }
    });
  }
  ...
  async _postState() {
    const targets = [this._view, this._editorPanel].filter(Boolean);
    ...
    for (const target of targets) target.webview.postMessage(state);
  }
