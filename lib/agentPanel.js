...
const LOCAL_TOOL_NAMES = new Set([
  'read_file', 'read', 'list_files', 'list',
  'run_in_terminal', 'bash',
  'edit', 'write', 'apply_patch', 'replace_string_in_file',
]);
...
class AgentPanelProvider {
  constructor(agentLoop, getHealth) {
    ...
    this._toolMessages = new WeakMap();
    this._terminal = null;
    vscode.window.onDidCloseTerminal(t => {
      if (t === this._terminal) this._terminal = null;
    });
    ...
    agentLoop.on('toolCall', call => {
      const msg = { role: 'tool', name: call.name, args: call.args, status: 'running', ts: Date.now() };
      this._messages.push(msg);
      this._toolMessages.set(call, msg);
      this._postState();
    });
    agentLoop.on('toolResult', ({ call, result }) => {
      const msg = this._toolMessages.get(call);
      if (!msg) return;
      const isUnknownLocalTool = !LOCAL_TOOL_NAMES.has(msg.name) && result?.error === `Unknown tool: ${msg.name}`;
      msg.status = (result?.success === false && !isUnknownLocalTool) ? 'error' : 'done';
      msg.result = result;
      this._postState();
    });
    ...
    webviewView.webview.onDidReceiveMessage(async msg => {
      switch (msg.type) {
        ...
        case 'openTui':
          ...
          break;
        case 'runInTerminal':
          this._handleRunInTerminal(msg);
          break;
        case 'continueInTerminal':
          this._handleContinueInTerminal();
          break;
        case 'newChat':
          this._messages = [];
  ...
  _getTerminal() {
    if (this._terminal && this._terminal.exitStatus === undefined) return this._terminal;
  ...
  _sendToTerminal(command) {
    const getConfig = () => vscode.workspace.getConfiguration();
    const terminal = this._getTerminal();
    terminal.show(true);
    terminal.sendText(buildTerminalCommand(getConfig, command));
  }

  _pushNotice(content) {
    this._messages.push({ role: 'notice', content, ts: Date.now() });
    this._postState();
  }

  _handleRunInTerminal(msg) {
    const text = (msg.text || '').trim();
    if (!text) return;

    let fullText = text;
    if (msg.contextFiles && msg.contextFiles.length > 0) {
      fullText += '\n\nContext files: ' + msg.contextFiles.join(', ');
    }

    const parts = ['opencode', 'run'];
    if (msg.agent || this._selectedAgent) parts.push('--agent', shellQuote(msg.agent || this._selectedAgent));
    if (msg.model || this._selectedModel) parts.push('--model', shellQuote(msg.model || this._selectedModel));
    if (this.agentLoop.sessionId) parts.push('--session', shellQuote(this.agentLoop.sessionId));
    parts.push(shellQuote(fullText));

    this._sendToTerminal(parts.join(' '));

    this._messages.push({ role: 'user', content: text, ts: Date.now() });
    this._pushNotice('Running in terminal — output will appear in the "OpenCode Agent" terminal.'
      + (this.agentLoop.sessionId ? ' Continuing session ' + this.agentLoop.sessionId.slice(0, 8) + '.' : ''));
  }

  _handleContinueInTerminal() {
    const sessionId = this.agentLoop.sessionId;
    this._sendToTerminal(sessionId ? 'opencode --session ' + shellQuote(sessionId) : 'opencode');
    this._pushNotice(sessionId
      ? 'Opened the TUI in the terminal, resuming session ' + sessionId.slice(0, 8) + '.'
      : 'Opened the OpenCode TUI in the terminal.');
  }
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tool-card-head { display: flex; align-items: center; gap: 6px; min-width: 0; cursor: pointer; user-select: none; }
    .tool-chevron {
      width: 10px; flex-shrink: 0; font-size: 9px;
      color: var(--vscode-descriptionForeground);
      transition: transform 0.12s; display: inline-block;
    }
    .tool-chevron.open { transform: rotate(90deg); }
    .tool-card-body { margin-top: 4px; display: flex; flex-direction: column; gap: 6px; }
    .tool-body-label {
      font-size: 0.85em; font-weight: 600; margin-bottom: 2px;
      color: var(--vscode-descriptionForeground);
    }
    .tool-body-section pre {
      background: var(--vscode-editor-background);
    ...
    .tool-icon.done { color: var(--vscode-testing-iconPassed, #4caf50); }
    .tool-icon.error { color: var(--vscode-errorForeground); }
    ...
    .tool-diffstat { flex-shrink: 0; font-family: var(--vscode-editor-font-family); }
    .tool-diffstat .added { color: var(--vscode-gitDecoration-addedResourceForeground, #4caf50); }
    .tool-diffstat .removed { color: var(--vscode-gitDecoration-deletedResourceForeground, #f44336); margin-left: 4px; }
    .tool-error-text {
      color: var(--vscode-errorForeground); font-size: 0.95em; padding-left: 18px;
    }
    .msg.error .msg-body {
    ...
    .term-btn {
      ...
      display: flex; align-items: center; justify-content: center;
      font-family: var(--vscode-editor-font-family); margin-right: 4px;
    }
    .term-btn:hover:not(:disabled) { background: var(--vscode-list-hoverBackground); color: var(--vscode-foreground); }
    ...
    #continue-term-btn:hover { background: var(--vscode-list-hoverBackground); color: var(--vscode-foreground); }
    .cancel-action {
...
<body>
    ...
    <div id="input-area-welcome">
      <div class="input-box">
        ...
        <div class="input-toolbar">
          ...
          <button class="term-btn" id="term-welcome" title="Run in terminal (Ctrl/Cmd+Enter)">&gt;_</button>
  ...
  <div id="chat">
    <div id="chat-header">
      ...
      <button id="continue-term-btn" style="margin-left:auto;" title="Resume this session in the OpenCode TUI">&gt;_ Terminal</button>
      <button id="new-chat-btn" style="margin-left:0;">+ New</button>
    </div>
    <div id="messages"></div>
    <div id="input-area-chat">
      <div class="input-box">
        ...
        <div class="input-toolbar">
          ...
          <button class="term-btn" id="term-chat" title="Run in terminal (Ctrl/Cmd+Enter)">&gt;_</button>
...
(function() {
  ...
  function toolTarget(m) {
    var a = m.args || {};
    if (m.name === 'run_in_terminal' || m.name === 'bash') return a.command || a.cmd || '';
    return a.path || a.file || a.filepath || '';
  }

  function toolDiffStat(m) {
    var a = m.args || {};
    if (a.old_string !== undefined && a.new_string !== undefined) {
      return { added: String(a.new_string).split('\n').length, removed: String(a.old_string).split('\n').length };
    }
    if ((m.name === 'write' || m.name === 'apply_patch') && typeof a.content === 'string') {
      return { added: a.content.split('\n').length, removed: 0 };
    }
    return null;
  }
  ...
  function toolLabel(name) {
    var labels = {
      ...
      run_in_terminal: 'Run', bash: 'Run',
      edit: 'Edit', write: 'Write',
      apply_patch: 'Edit', replace_string_in_file: 'Edit',
    };
    return labels[name] || name;
  }

  function toolBodyHtml(m) {
    var html = '<div class="tool-body-section"><div class="tool-body-label">Input</div><pre>'
      + esc(JSON.stringify(m.args || {}, null, 2)) + '</pre></div>';
    if (m.result) {
      var out = m.result.content !== undefined ? m.result.content
        : m.result.files !== undefined ? m.result.files.join('\n')
        : m.result.message !== undefined ? m.result.message
        : '';
      if (out) {
        var preview = String(out).length > 2000 ? String(out).slice(0, 2000) + '\n…' : String(out);
        html += '<div class="tool-body-section"><div class="tool-body-label">Output</div><pre>' + esc(preview) + '</pre></div>';
      }
    }
    return html;
  }
  ...
  function renderMessages(messages) {
    ...
    for (var i = 0; i < messages.length; i++) {
      var m = messages[i];
      if (m.role === 'tool') {
        var icon = m.status === 'running' ? '' : (m.status === 'error' ? '✕' : '✓');
        var target = toolTarget(m);
        var diff = toolDiffStat(m);
        var diffHtml = diff ? '<span class="tool-diffstat"><span class="added">+' + diff.added + '</span>'
          + (diff.removed ? '<span class="removed">-' + diff.removed + '</span>' : '') + '</span>' : '';
        var isOpen = !!expandedTools[m.ts];
        html += '<div class="msg tool"><div class="tool-card">'
          + '<div class="tool-card-head" data-ts="' + m.ts + '">'
          + '<span class="tool-chevron' + (isOpen ? ' open' : '') + '">&#9656;</span>'
          + '<span class="tool-icon ' + m.status + '">' + icon + '</span>'
          + '<span class="tool-name">' + esc(toolLabel(m.name)) + '</span>'
          + '<span class="tool-target" title="' + esc(target) + '">' + esc(target) + '</span>'
          + diffHtml
          + '</div>'
          + (isOpen ? '<div class="tool-card-body">' + toolBodyHtml(m) + '</div>' : '')
          + (m.status === 'error' && m.result && m.result.error ? '<div class="tool-error-text">' + esc(m.result.error) + '</div>' : '')
          + '</div></div>';
        continue;
      }
      if (m.role === 'notice') {
        html += '<div class="msg notice"><div class="msg-body"><span>&gt;_</span><span>' + esc(m.content) + '</span></div></div>';
        continue;
      }
      if (m.role === 'error') {
    ...
    messagesEl.querySelectorAll('.tool-card-head').forEach(function(head) {
      head.addEventListener('click', function() {
        var ts = head.getAttribute('data-ts');
        expandedTools[ts] = !expandedTools[ts];
  ...
  document.getElementById('term-welcome').addEventListener('click', function() { runInTerminal(inputWelcome); });
  document.getElementById('term-chat').addEventListener('click', function() { runInTerminal(inputChat); });
  document.getElementById('continue-term-btn').addEventListener('click', function() {
    vscode.postMessage({ type: 'continueInTerminal' });
  });
...
module.exports = { AgentPanelProvider, shellQuote };
