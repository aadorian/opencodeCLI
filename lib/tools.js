 ...
 * Actual format: {type:"tool_use", part:{tool:"read", state:{input:{...}}}}
 * @param {object} event
 ...
 * Actual format: {type:"text", part:{text:"Hello!"}}
 * @param {object} event
 * @returns {string}
 */
function parseTextFromEvent(event) {
  if (!event || typeof event !== 'object') return '';
  // actual opencode --format json output: event.part.text
  if (event.type === 'text' && event.part && typeof event.part.text === 'string') return event.part.text;
  // fallbacks
