...
- Create new agent instruction files in `.github/agents/` (GitHub Copilot format) and `.claude/agents/` (Claude Code format) for OpenCode-specific automation needs.
- Ensure new agents follow the repo's conventions for extension commands, CLI dispatch, walkthroughs, and harness integration.
...
- **GitHub Copilot agents** go in `.github/agents/` with `target: github-copilot` frontmatter.
- **Claude Code agents** go in `.claude/agents/` using `Read`, `Edit`, `Write`, `Bash` tool names — no `target` field.
- Both formats share the same system prompt body; only the frontmatter differs.

## Common tasks

- New agent command: update `package.json`, `extension.js`, and `test/extension.test.js`.
- New agent workflow: add instructions under `.github/agents/` and `.claude/agents/`, and optionally document in `AGENTS.md`.
- CLI integration: use `lib/cli.js` and `lib/env.js` for terminal dispatch.

This agent generates and evolves custom OpenCode agents — it does not run the OpenCode CLI itself. Keep instructions clear, actionable, and aligned with the existing custom agent model.
